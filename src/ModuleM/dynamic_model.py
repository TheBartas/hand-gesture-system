...
from keras.layers import Dropout, Dense, LSTM, Bidirectional, LayerNormalization
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import LabelEncoder
from ModuleM.paths import DYNAMIC_PATH, MODEL_KERAS_PATH, MODEL_TFLITE_PATH, INFO_PATH, MODEL_JSON_PATH


class DynamicGestureModel(HandGestureModel) :
    def __init__(
        self, 
        camera=0,
        model_complexity=1,
        ...
        epochs=20,
        batch_size=34,
        instance_name='dynamic_default',
        model_file_name='dynamic_hand_gesture_model_default',
        target_frames = 40,
        ) : 
        super().__init__(
            camera=camera,
    ...
    def init_data_files(self) : 
        ...
        names = [gesture['name'] for gesture in data if gesture['type'] == 'dynamic']

        for name in names:
            path = DYNAMIC_PATH / f'{name}_data.csv'
            if not path.exists():
    ...
    def collect_data(self) : 
        ...
        dynamic_gestures = data[data['type'] == 'dynamic']
        print(dynamic_gestures)
        ...
        sample = 0
        sequence = []
        path = DYNAMIC_PATH / f'{gesture_name}_data.csv'
        ...
        with mp.solutions.hands.Hands(
            ...
            while cap.isOpened():
                ...
                frame_counter+=1
                fps = self.__calculate_fps(frame_counter, start_time)
                ...
                if results.multi_hand_landmarks:
                    for hand_landmarks in results.multi_hand_landmarks:
                        ...
                        if is_recording:
                            sequence.append(self.__extract_points(hand_landmarks))
                
                cv2.putText(image, f'FPS: {fps:.2f}', (180, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2,
                    cv2.LINE_AA)
                cv2.imshow('M-Module - dynamic hand gestures', image)  
                ...
                if key == ord('t'):
                    is_recording = True
                    print('[info] Start recording gesture.')

                if key == ord('e'):
                    is_recording = False
                    print(f'[info] Stop recording gesture.')

                    seq_resampled = self.__prepare_data(self.__normalize(sequence))
                    self.__save_landmarks(path, gesture_name, seq_resampled)
                    sequence.clear()
                    sample+=1
                    print(f"[info] Data save! | Sample no. {sample}")

        cap.release()
        cv2.destroyAllWindows()

    def build_model(self) : 
        files = DYNAMIC_PATH.rglob('*.csv')
        ...
        X = X.reshape(X.shape[0] // self._target_frames, self._target_frames, 42)
        y = y[::self._target_frames]

        print("X shape:", X.shape)  # powinno być (num_samples, 32, 42)
        ...
        self._model = Sequential([
            LSTM(128, return_sequences=True, input_shape=(self._target_frames, 42)),
            Dropout(0.3),
        ...
        self._model.fit(X_train, y_train, epochs=self._epochs, validation_data=(X_test, y_test))
    ...
    def convert_to_tflite(self) : 
        ...
        self._model_tflite = converter.convert()  
    ...
    def __prepare_data(self, seq) :
        print(f'[info] seq before down-sampling len: {len(seq)}')
        curr_len = len(seq)
        
        if curr_len > self._target_frames :
            indexes = [int((curr_len / self._target_frames) * k) for k in range(self._target_frames)]
            return seq[indexes]

        if curr_len < self._target_frames :
            last_frame = np.expand_dims(seq[-1], axis=0)
            padding = np.repeat(last_frame, self._target_frames - curr_len,axis=0)
            return np.concatenate([seq, padding], axis=0)
        return seq
    
    def __save_landmarks(self, path, gesture_name, seq_resampled) :
        with open(path, 'a', newline="") as file:
            writer = csv.writer(file)
            for row in seq_resampled:
                writer.writerow([gesture_name] + row.tolist())

    def __calculate_fps(self, frame_counter, start_time) :
        fps = 0
    ...
    def __normalize(self, seq) :
        seq = np.stack(seq)

        wrist = seq[0,0]
        normalized = seq - wrist

        return normalized.reshape(normalized.shape[0], -1)

    def __extract_points(self, hand_landmarks):
        return np.array([[lm.x, lm.y] for lm in hand_landmarks.landmark], dtype=np.float32)
