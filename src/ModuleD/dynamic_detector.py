...
class DynamicGestureDetector(HandGestureDetector) :
    def __init__(
            self,
            model : str = "dynamic_hand_gesture_model",
            dict : dict = None,
        ) :
    ...
    def detect(self, hand_seq) :
        input_data = np.array(hand_seq, dtype=np.float32).reshape(self._input_details[0]['shape'])
    ...
    def extract_points(self, hand_landmarks):
